...
- Tema Liquid Glass (beta, Jul 2026): toggle Settings → Tampilan (`liquidThemeProvider`, pref `liquid_theme`). `FtColors.liquid` statis (pola sama brightness, flip → `ftRebuildAllWidgets`). Saat ON: scaffold transparan + `FtLiquidBackground` global di `app.dart` (cool-color drift loop 20s), chrome pakai `ui/ft_glass.dart` + `ui/ft_glass_fx.dart` (blur 18 + saturasi 1.18 + lensa wallpaper + rim specular; tint 0.12 light / 0.18 dark; high-contrast & reduce-motion dihormati). Glass HANYA chrome (bottom/side nav, action sheet); semua `FtCard` solid, radius 12, hairline, tanpa shadow. Selected nav lens tetap transparan; gerak easeOutCubic 320ms. FtTapScale respons 80ms, settle 220ms tanpa elastic wobble. Lab debug: `/dev/liquid`. Regression: `test/widget/ft_glass_test.dart`.
- Perf Liquid (Jul 2026, jangan diregres): scene dirender SEKALI per langkah (~12 fps, `Timer` — BUKAN Ticker: ticker menjadwalkan frame tiap vsync dan web/canvaskit re-raster semua) ke `ui.Image` bersama (`LiquidFrame`, resolusi dibudget ±0.35 MP); background + semua lensa cuma `drawImageRect` (tanpa saveLayer — alpha via paint). Painter WAJIB baca `frame.image` di paint() (bukan capture saat build; image lama di-dispose tiap langkah). Tidak ada pointer-move listener per kartu, sweep periodik, atau blur di content layer. Timer stop saat app paused / reduce-motion / liquid OFF. PENTING: `routerProvider` JANGAN `ref.watch` auth/userDoc di body (GoRouter dibuat ulang tiap emit → navigasi terlempar ke '/'); pakai `ref.read` dalam redirect + `refreshListenable`. Toggle tema: `themeAnimationDuration: Duration.zero` di app.dart WAJIB; statics di-sync dari resolusi themeMode (bukan Theme.of) + post-frame rebuild bila brightness berubah tanpa toggle (mode system).
...
### References
- `DESIGN.md` → source of truth visual. `claude-design/` hanya arsip prototype lama; jangan dijadikan arah desain.
