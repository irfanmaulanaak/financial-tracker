...
- Cicilan tetap non-kartu (Agu 2026): `households/{hid}/obligations` (leasing/KPR) — layar `/obligations`, bayar via `payMonth` (saldo rekening turun, TIDAK makan budget kategori). DSR = (cicilan tetap + cicilan kartu `monthsTotal>1`) / gaji stabil (`stableSalaryProvider` = rata-rata gaji siklus lengkap, `core/stable_income.dart`), batas OJK 30% (`core/disposable_income.dart`). Uang siap pakai = gaji stabil − cicilan → banner home (warning DSR>30% / overcommit budget / info cicilan jalan). Net worth hitung `outstandingPrincipal` HANYA bila diisi (monthly×sisa bulan ≠ pokok). Tenor 1× = tagihan sekali bayar, bukan cicilan (`monthlyInstallmentDsrTotal`).
